...
: G4VUserDetectorConstruction(),
  ...
  fPdbFileName=G4String("tyrosine.pdb");
  printf("variable asociada \n");
  fPdbFileStatus=0;
  fChosenOption=11;
  fpDefaultMaterial=0;
  fpWaterMaterial=0;
...
G4VPhysicalVolume* DetectorConstruction::Construct(){
  ...
  printf("Total DFT energy in GeantV: %f with %f ns \n",  energy ,Etime);
  return worldPV;
}
...
G4double worldSize  = 1000*1*angstrom;
  ...
  G4VSolid* worldS= new G4Box("World",           
              worldSize/2, worldSize/2, worldSize/2); 

  G4LogicalVolume*  worldLV  = new G4LogicalVolume(  worldS, fpDefaultMaterial,  "World");  
...
void DetectorConstruction::AtomisticView(G4LogicalVolume* worldLV,
  ...
  G4LogicalVolume* atomLV_O = new G4LogicalVolume(
      atomS_O, fpWaterMaterial, "atomLV_O"); 
  ...
  G4LogicalVolume* atomLV_N = new G4LogicalVolume(
      atomS_N, fpWaterMaterial, "atomLV_N"); 
...
void DetectorConstruction::ResiduesView(G4LogicalVolume* worldLV,
  ...
  while (barycenterListTemp)  {
    ...
    tubS1_ZZ  = new G4Tubs( "Cylinder",  0.,  0.5*angstrom,
                              std::sqrt (
