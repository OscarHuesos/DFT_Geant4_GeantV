...
void DFT::normalize(Molecule& Mol){
...
for((*atr)->shell1 = (*atr)->orbitals.begin();
  ...
  (*(*atr)->shell1).NAC[i].N=pow(2.0*((*(*atr)->shell1).NAC[i].A)/pi, 0.75)*
  (pow(4.0*((*(*atr)->shell1).NAC[i].A), 0.5*(l1+l2+l3))/sqrt(n1*n2*n3) );
...
V=fAtract.Nuclear(Mol);
return;
}


void DFT::rep(Molecule& Mol ){
J=fRepulsion.pairing(Mol);
return;
}
...
// 0 sera lda, 1 gga, 
G4int functional = 1;
Vxc = fExchange.XCpotential(Mol, D,  W , functional);
